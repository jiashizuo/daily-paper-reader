---
title: Integrating Mechanistic and Data-Driven Models for Neurological Disorders through Differentiable Programming
title_zh: 通过可微编程整合神经疾病的机理与数据驱动模型
authors: "Shah Pallav Dhanendrakumar, Saikat Pal, Sitikantha Roy"
date: 2026-06-04
pdf: "https://arxiv.org/pdf/2606.06094v1"
tags: ["query:fmri-brain-network"]
score: 6.0
evidence: 神经影像，人工智能，神经系统疾病混合建模
tldr: 神经疾病建模中，纯机理模型计算昂贵且简化假设多，纯数据驱动模型缺乏可解释性和泛化能力。本文提出混合建模策略，通过并行、串行和并行-串行架构融合深度学习与物理求解器，包括残差建模、神经常微分方程和求解器加速三种方法。混合模型在脑肿瘤、阿尔茨海默病和中风等疾病中提升了诊断准确性和治疗规划效果。该框架为个性化神经疾病建模提供了高效、可解释的新途径。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2606.06094v1/fig-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1008, \"height\": 666}, {\"url\": \"assets/figures/arxiv/2606.06094v1/fig-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1652, \"height\": 497}, {\"url\": \"assets/figures/arxiv/2606.06094v1/fig-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 1712, \"height\": 474}, {\"url\": \"assets/figures/arxiv/2606.06094v1/fig-004.webp\", \"caption\": \"\", \"page\": 0, \"index\": 4, \"width\": 1050, \"height\": 646}, {\"url\": \"assets/figures/arxiv/2606.06094v1/fig-005.webp\", \"caption\": \"\", \"page\": 0, \"index\": 5, \"width\": 1748, \"height\": 358}, {\"url\": \"assets/figures/arxiv/2606.06094v1/fig-006.webp\", \"caption\": \"\", \"page\": 0, \"index\": 6, \"width\": 1755, \"height\": 461}, {\"url\": \"assets/figures/arxiv/2606.06094v1/fig-007.webp\", \"caption\": \"\", \"page\": 0, \"index\": 7, \"width\": 1602, \"height\": 430}, {\"url\": \"assets/figures/arxiv/2606.06094v1/fig-008.webp\", \"caption\": \"\", \"page\": 0, \"index\": 8, \"width\": 1396, \"height\": 385}, {\"url\": \"assets/figures/arxiv/2606.06094v1/fig-009.webp\", \"caption\": \"\", \"page\": 0, \"index\": 9, \"width\": 1750, \"height\": 993}]"
tables_json: "[{\"url\": \"assets/tables/arxiv/2606.06094v1/table-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1727, \"height\": 594}]"
motivation: 纯机理模型计算昂贵且简化假设多，纯数据驱动模型缺乏可解释性和泛化能力，需融合两者优势。
method: 提出并行、串行和并行-串行三种混合架构，结合残差建模、神经常微分方程和求解器加速方法。
result: 混合模型在脑肿瘤、阿尔茨海默病和中风等疾病中提升了诊断准确性和治疗规划效果。
conclusion: 混合建模优于单一方法，为个性化神经疾病建模提供高效、可解释的新途径。
---

## 摘要
计算建模、神经影像和人工智能的进步正在革新神经疾病的建模，以改进诊断、预后和治疗规划。机理模型为疾病提供了宝贵的科学见解，但在实践中，它们往往因假设而简化，或计算成本高且求解缓慢。然而，纯数据驱动方法虽然提供了速度和可扩展性，但需要大量高质量数据进行训练，并且通常存在可解释性和泛化问题。本视角论文对混合建模策略进行了结构化概述，这些策略将深度学习模型与基于物理的求解器相结合，并分为并行、串行和并行-串行架构。重点介绍了三种主要方法：用于缺失或不完整物理的残差建模、用于连续时间动力学近似的神经常微分方程（NODE），以及通过神经近似加速传统求解器的求解器在环方法。这些混合模型整合了基于控制微分方程的公式和深度学习，以表征神经疾病的演变，并有望实现先进的个性化神经建模。此外，本研究探索并提出了不同的混合配置，以提高诊断准确性、预测疾病进展，并为一系列神经疾病的治疗策略提供信息。这些能力优于单独的机理或纯数据驱动方法，使混合建模成为一种强大的工具，特别是在涉及脑肿瘤、阿尔茨海默病和中风等神经疾病的进展和治疗反应建模的应用中。

## Abstract
Advances in computational modeling, neuroimaging, and artificial intelligence are revolutionizing the modeling of neurological disorders for improved diagnostics, prognosis, and treatment planning. Mechanistic models provide valuable scientific insight into the disorders, but in practice they are often simplified with assumptions or computationally expensive and slow to solve. However, while purely data driven approaches provide speed and scalability, they require large, high quality data to train and generally suffer from interpretability and generalization issues. This perspective paper presents a structured overview of hybrid modeling strategies, which combine deep learning models with physics based solvers, and are categorized into parallel, series, and parallel-series architectures. Three main approaches that have been emphasized are residual modeling for missing or incomplete physics, Neural Ordinary Differential Equations (NODEs) for continuous time dynamics approximation, and solver in the loop that accelerates traditional solvers with neural approximations. These hybrid models integrate the governing differential equation based formulations and deep learning to characterize the evolution of neurological disorders, and promise advanced personalized neurological modeling. In addition, the study explores and proposes different hybrid configurations to improve diagnosis accuracy, predict disease progression, and inform treatment strategies across a range of neurological disorders. These capabilities outperform standalone mechanistic or purely data driven approaches, making hybrid modeling a powerful tool, especially in applications involving modeling the progression and treatment responses in neurological conditions such as brain tumors, Alzheimer's disease, and stroke.