...
tldr: 现有皮层分区方法多依赖静息态fMRI，忽略了任务态数据提供的功能特化信息。本文提出mRBM-HBP框架，通过多类受限玻尔兹曼机融合静息态与任务态fMRI，实现高效灵活的个体和群体皮层分区。结果表明，融合分区在准确性和可靠性上优于单一模态方法，尤其在个体数据有限时显著提升个体特异性。该工作为精准脑功能图谱构建提供了新途径。
source: biorxiv
selection_source: fresh_fetch
motivation: 现有皮层分区方法主要依赖静息态fMRI，未能充分利用任务态数据中互补的功能特化信息，且难以整合异质性数据集。
method: 提出mRBM-HBP，一种可扩展的层次贝叶斯框架，利用多类受限玻尔兹曼机建模空间依赖，融合静息态与任务态fMRI进行群体和个体皮层分区。
result: mRBM-HBP在计算成本大幅降低的同时达到与现有方法相当的性能；融合分区提高了准确性和可靠性，尤其在个体数据有限时增强了个体特异性。
conclusion: 融合静息态与任务态fMRI可提升皮层功能分区的精度，为个体化脑网络研究提供更优工具。
---

## 摘要
个体特异性皮层分区能够表征常被群体图谱掩盖的脑网络组织，对基础神经科学和转化应用具有广泛意义。然而，现有方法主要依赖静息态fMRI，未能充分利用任务诱发数据——后者提供了关于功能特化的互补信息。这一局限性部分反映了整合不同任务设计、样本量和皮层覆盖范围的异质数据集的挑战。本文提出mRBM-HBP，一种可扩展的分层贝叶斯框架，通过引入多项受限玻尔兹曼机建模空间依赖性，实现了跨不同数据集对静息态和任务态fMRI的高效灵活整合，并推断出群体水平和个体水平的皮层分区。我们证明，mRBM-HBP在显著降低计算成本的同时，达到了与基于静息态的最先进分区方法相当的性能。通过整合大规模任务态fMRI数据集，我们推导出基于任务的分区，并表明静息态和任务条件揭示了基本一致的宏观网络，而任务数据提供了功能边界的状态特异性细化。此外，融合静息-任务的群体图谱提高了推断分区的准确性、可靠性和个体特异性，尤其在个体水平数据有限时。这些结果表明，整合静息态和任务态fMRI增强了脑功能组织的精确映射。
